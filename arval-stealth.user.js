...
function hideBNPLoader() {
  ...
  // видаляємо також можливий хост, якщо лоадер було переміщено всередину модалки
  document.getElementById('bnp-loader-host')?.remove();
}
// ==== /BNP Paribas Loader ===================================================
...
// ==== showAllRedsModal (з лоадером у модалці) ====
async function showAllRedsModal() {
  const modal = ce('div', { className: 'arv-modal' });
  modal.innerHTML = `
    <div class="arv-modal__panel">
  ...
  // показуємо бренд-лоадер усередині тіла модалки
  showBNPLoader(body);
